...
import { PinLoginGuardService } from '../staff-auth/pin-login-guard.service';
...
export class IdentityController {
  constructor(
    ...
    private readonly pinLogin: PinLoginGuardService,
  ) {}
  ...
  async staffLogin(
    ...
    this.pinLogin.assertNotLocked({ email, ip: clientIp, correlationId });

    const staff = await this.prisma.staffMember.findUnique({
      where: { email },
    });
    if (!staff || !(await bcrypt.compare(body.password || '', staff.passwordHash))) {
      this.pinLogin.recordFailure({ email, ip: clientIp, correlationId });
      throw new UnauthorizedException('Invalid email or password');
    }

    this.pinLogin.recordSuccess({ email, ip: clientIp });
