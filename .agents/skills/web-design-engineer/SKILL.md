...
**This skill outputs standalone HTML/CSS/JS/React artifacts — mockups, prototypes, comparisons, decks. It does not write production Angular code.** For Lekki:

- Use it for: exploring a redesign direction before touching real code (e.g. "show me 3 variants of the guest menu card"), one-off comparison/critique artifacts, internal decks (investor updates, roadmap reviews), throwaway prototypes to validate an interaction before it's built. When the direction is approved, `frontend-design` (`.agents/skills/frontend-design/`) is the skill that ports it into the real Angular components under `apps/web/src/app/`.
- **Do not use it to edit files under `apps/web/`.** That codebase is Angular (standalone components) + SCSS, not React/Babel/Tailwind — this skill's technical specifications (React+Babel CDN, `const styles`, Tweaks panel, etc.) target its own throwaway HTML artifacts, not Lekki's production stack.
- **Treat Lekki's brand as already established — this is not a greenfield-brand task.** Skip Step 3a/3's "declare a new design system" as if starting from nothing. Instead, paste the real values directly into the Design Decisions block:
  - Palette: Surgical White & Rose-Gold — `#ffffff` canvas, `#0f172a` ink, `#d7a14a` gold accent (rationed to primary CTA / focus / selected only). Full spec: `docs/ux/lves.md`, tokens in `apps/web/src/styles/_tokens.scss`.
  - Typography: Sora (UI), Fraunces (display/headline moments only) — both already loaded via Google Fonts, see `apps/web/src/index.html`.
  - Radius/shadow/motion: 24px card / 14px button / 12px input radius; soft warm-tinted shadows; motion is fade/flow/rise/settle only — never bounce, pop, shake, or spin (`docs/ux/leos-motion-system.md`, Frozen).
  - Brand assets: `apps/web/public/brand/` (logo marks, lockup, splash photo) — use the real files, don't redraw the mark.
  - Reference points: Experience feels like *Apple Wallet · Uber Eats · Airbnb*; Studio feels like *Stripe · Linear · Google Admin* (`docs/ux/lves.md`). This maps directly onto two recipes already in this skill's own library — `references/style-recipes/apple-hig.md` and `references/style-recipes/linear.md` — worth reading for concrete technique even though Lekki's actual tokens take precedence over either recipe's specific values.
  - Do not run the "Fallback: Design Direction Advisor" (3-schools picker) for Lekki work unless the user explicitly wants to explore a direction outside LVES — the direction is already decided.
- **No dark mode.** Lekki is light-only by design (Studio and Experience are both "never dark"). Skip dark-mode toggles unless explicitly requested as a real product decision.
- If a mockup built here is heading toward production, name the target Angular component/page it corresponds to (e.g. "this becomes `leos-menu-card` in `apps/web/src/app/leos/menu-card.component.ts`") so the handoff to `frontend-design` is concrete instead of a re-guess.
...
- **Product context**: What product? Target users? Existing design system / brand guidelines / codebase? (For Lekki: see Lekki Context above — the brand and tokens already exist.)
...
> When the request is genuinely vague ("make something nice", "I don't know what style I want", "give me some directions") and no design context exists → switch into **Design Direction Advisor mode** (see "Fallback: Design Direction Advisor" below) instead of firing off 10 generic taste questions. For Lekki, this fallback essentially never triggers — the direction is already set.
...
2. **Existing pages of the user's product** → proactively ask whether you can review them (for Lekki: `apps/web/src/app/pages/`, `docs/ux/lves.md`)
...
- **Logo is non-negotiable** — if you can't source it after a real attempt, **stop and ask the user**, don't proceed with a colored rectangle. For Lekki: the real marks are in `apps/web/public/brand/` — use those, don't ask or redraw.
- **Color hex codes alone are not a brand** — they're the cheapest part of the identity
- Capture all assets in a `brand-spec.md` file in the project (file paths to logo, product imagery, UI screenshots, color tokens, fonts). All HTML must reference these via `<img src="…">`, not redraw them. For Lekki, this can simply point at `docs/ux/lves.md` + `apps/web/public/brand/` rather than re-deriving one.
...
Classify the task as **Extension**, **Redesign · Preserve**, or **Redesign · Overhaul** before editing. Read `references/redesign-protocol.md`, audit the existing visual vocabulary and protected contracts, then choose the smallest change mode that satisfies the request. New elements in Extension mode should be indistinguishable from the originals. For Lekki, also cross-check `docs/NORTH-STAR.md` / `docs/ux/current-product-state.md` — Setup Engine v1, the Experience/Studio shells, and the core guest journey are frozen against Overhaul-level change regardless of what this protocol would otherwise allow.
...
Use the dials as decision variables, not decorative scores. They must affect layout variation, motion, content per viewport, real-asset effort, and preservation strictness. Read `references/design-calibration.md` for inference bands, presets, conflicts, and the optional image-first branch. For Lekki work, `motion-intensity` should default low (LVES motion is deliberately restrained) unless the artifact is explicitly for the marketing site (see `build-awwwards-quality-sites` skill, which owns that higher-motion register).
...
- Anchor / recipe (if any): [for Lekki, default to "Lekki LVES 2.0 — Surgical White & Rose-Gold", not a library recipe, unless exploring an explicit alternative]
...
**For Lekki specifically**: this almost never applies — the direction (LVES 2.0, Surgical White & Rose-Gold) is already decided. Only use this fallback if the user explicitly wants to explore something outside that system as a deliberate experiment.
...
Note for Lekki: LVES 2.0 already sits at the intersection of **Editorial / minimalist** (Apple HIG, quiet luxury) and **Warm humanist** (approachable, hospitality-driven) — that blend is the reason it isn't a straight match for any single row above.
...
- For branded work, all real brand assets live under `assets/<brand>-brand/` and are referenced from `brand-spec.md`. For Lekki, prefer referencing `apps/web/public/brand/` directly.
...
| Inter / Roboto / Arial / Fraunces / system-ui as display | Too common; reads as "demo page" rather than "designed product" | The brand spec specifies these (and usually with custom adjustments) — **Lekki's brand spec does specify Fraunces as its display face**, so this is the one row where Lekki is the explicit exception |
...
**No emoji by default.** Only use emoji when the target design system/brand itself uses them (e.g., Notion, early Linear, certain consumer brands), and match their density and context precisely. Lekki's own visual language explicitly avoids emoji as UI affordances (`docs/LEK-026-leds-visual-language.md`) — treat this as a hard no for Lekki work, not just the general default.
...
- Missing logo → **stop and ask the user** (see Asset Protocol); never substitute "brand name in a colored box" for a logo on branded work — for Lekki, the real logo is already in `apps/web/public/brand/`, so there should be no reason to reach this case
...
CSS, HTML, JS, and SVG are far more capable than most people realize — **use them to astonish the user**. For Lekki product-facing mockups, "stun" still means restrained per LVES — the astonishment is in craft and clarity, not spectacle; save the higher-intensity version of this principle for marketing-site work under `build-awwwards-quality-sites`.
...
| Mobile mockups | Touch targets ≥ 44px (Lekki's own minimum is higher — 52px, `--leos-touch-min`) |
...
- Provide dark/light mode toggle (skip for Lekki — light-only)
...
| Custom typography | Google Fonts (avoid Inter / Roboto / Arial / Fraunces / system-ui as display — except Lekki, whose brand spec specifies Fraunces) |
...
- [ ] **If the task is branded**: `brand-spec.md` exists (or points at `docs/ux/lves.md` for Lekki); logo is real (not a colored rectangle); product imagery is real (not a CSS silhouette) for hardware; UI screenshots are real for digital products
...
- [ ] For Lekki work: no dark-mode toggle added unless explicitly requested; motion stays restrained (fade/flow/rise/settle) unless this is explicitly marketing-site work
...
| User named an anchor ("Linear-style" / "Aesop feeling") → load **only that one file** | `references/style-recipes/<anchor>.md` (e.g., `linear.md`, `aesop.md`, `apple-hig.md` — the two closest to Lekki's own references) |
